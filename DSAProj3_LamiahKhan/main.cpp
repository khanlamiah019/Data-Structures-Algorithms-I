...
// second resubmission approach: make the hash tables faster
...
//the first and last names given are placed into hash-maps -->easier for data in general. 
unordered_map<string, unsigned int> firstName = {
...
unordered_map<string, unsigned int> lastName = {
...
//determines what data type the input is
//citation:  https://stackoverflow.com/questions/30569789/to-check-type-of-input-in-c 
string dataFun(list<Data *> &l){
  list<Data *>::iterator endElem;
  list<Data *>::iterator nextElem;
  nextElem = l.begin();
  nextElem++;
  endElem = l.end();
  endElem--;
  if ((*l.begin())->lastName == (*endElem)->lastName){
    return "T4";}
  else if ((*l.begin())->lastName == (*nextElem)->lastName){
    return "T3";}
...
bool compD(const Data* info1, const Data* info2){
  const char *lastName1 = info1->lastName.c_str();
  const char *lastName2 = info2->lastName.c_str();
   // will return false if the members are not in descending order
   //https://stackoverflow.com/questions/59348144/in-c17-does-being-non-decreasing-necessarily-imply-being-sorted-even-when-us
    for ( ; *lastName1 == *lastName2; lastName1++, lastName2++) {
      if (*lastName1 == 0){
...
// Clears the original list
  ...
  // Sorts the sublist using the ssnCompare1 function 
    {nameSort[p1][p2].sort(compareSSN);   
    //merges the the sorted sublist back to the original list
      l.splice(l.end(), nameSort[p1][p2]); }}}
